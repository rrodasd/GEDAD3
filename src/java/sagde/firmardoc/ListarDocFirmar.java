...
public class ListarDocFirmar extends HttpServlet {
            ...
            try {
                ...
                if (codigoOrganizacion != null) {
                    if (codigoOrganizacion.equals("0000")) {
                        codigoOrganizacion = null;
                    }
                }
                listaPorRevisar = objRD.ObtenerListaDocumentosXRevisar("A", objbean.getVUSUARIO_CODIGO(), codigoOrganizacion);
            } catch (SQLException ex) {
                Logger.getLogger(ListarDocFirmar.class.getName()).log(Level.SEVERE, null, ex);
            }
            request.setAttribute("listaPorRevisar", listaPorRevisar);
            getServletContext().getRequestDispatcher("/gedad/firmardoc/I_PorFirmar.jsp").forward(request, response);
        }
    }
